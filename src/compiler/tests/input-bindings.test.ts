...
describe('compile — InputBindings / CommandBindings markup authoring', () => {
    test('KeyBinding[Key=…, Modifiers=…] lowers to plain-field assignment + push', () => {
        ...
        assert.match(js, /import \{[^}]*KeyBinding[^}]*\} from "@pragmatic-lab/mural\/framework\/commands\/input-binding\.js"/);
        assert.match(js, /import \{[^}]*\bKey\b[^}]*\} from "@pragmatic-lab/mural\/runtime"/);
    });
