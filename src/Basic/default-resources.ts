...
// consolidated ResourceDictionary — see Basic/basic.template.mu.
...
// Basic barrel (it instantiates Border / StackPanel / etc.) and the
...
// here — co-locating it would force a static import of
// `surface.template.mu.js`, which transitively pulls every Button
// subclass through Button's static block (`extends Button` TDZ).
// Surface controls call the helper from that sibling file directly.
...
export function defaultTemplate(klass: Function): ControlTemplate
{
    ...
    if (resource === undefined)
    {
        throw new Error(
            ...
            `Did you import the Basic barrel before constructing the control?`);
    }
