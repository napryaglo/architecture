...
Application[Theme = Material, Scheme = MaterialLight] {
    resources: {
        ...
        // host bootstrap via SetTheme). @Surface / @Primary etc. fall
        // through to dynamic-resource bindings, so a ToggleTheme() call
        // repaints the whole platform — shell chrome included.

        Border x:root [Background=@Surface]{
            // AdornerDecorator scopes adornment to the whole platform
            // root — any control inside can locate this layer via
            // AdornerLayer.GetAdornerLayer(this) and drop adorners here.
            AdornerDecorator {
