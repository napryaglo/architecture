...
// $Width × $Height containing the shape primitive (Fill=$FillBrush,
// Stroke=$Stroke) and a TextBlock label centered over it. The shape
// primitive class is the only per-Kind delta.
//
// Selection chrome lives in the SelectionResizeAdorner overlay rather
// than per-shape Stroke triggers — that frees the Stroke DP to carry
// the user's edited pen (set by ShapeFormatControl) without selection
// state masking it.
//
