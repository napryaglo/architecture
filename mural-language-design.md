...
import Controls from "@visualisation-sub/mural"
...
Compilation runs at build time. `.mu` files compile to plain JS modules that import directly against the existing runtime ([src/runtime/](src/runtime/), [src/Controls/](src/Controls/), [src/visual-engine/](src/visual-engine/)) and use it imperatively — no parser ships with the consumer's runtime bundle.
...
import { Border, TextBlock, Button, Window } from '@visualisation-sub/mural/Controls';
...
- [src/Controls/](src/Controls/) — the control library whose constructors the bind pass targets.
