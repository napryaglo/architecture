...
Surfaced by the post-shipping self-review against [m3-modernization-plan.md](m3-modernization-plan.md) (Phases 0-9 + Appendix C + Phase 2.5 / Phase 3.5 Tier-1 close-out) and the architecture review of the SplitButton popup-template fix against [theme-architecture.md](theme-architecture.md). Items 18.1-18.5 are *new* runtime / framework gaps that the Tier-1 work (SegmentedButton, ButtonGroup, SplitButton, FabMenu) exposed but couldn't fix in their own scope. 18.6-18.8 are plan-process debt. 18.9 lists the plan-explicit deferrals still awaiting demo motivation. 18.10-18.12 are theme-architecture-level caveats surfaced while making the SplitButton popup chrome conformant.
...
~~18.5. **Inline-author popup body in markup (SplitButton, future fly-outs).**~~ Closed by option (b) — see [completed-backlog.md § 18.G](completed-backlog.md). `SplitButton.PopupTemplate: ControlTemplate` DP wires the M3 popup chrome ([framework.resources.mu DefaultSplitButtonPopup](src/resources/framework.resources.mu)) the same way `MenuButton.TriggerTemplate` wires its inline trigger; `mountPopup` calls `tpl.Apply(this)` and `SetChild`s `MenuContent` into the named `PART_PopupBody` slot. Demo bootstrap now ships only the items `StackPanel`; no JS-side `@Token` lookups.
...
~~18.10. **Subtree `Scheme` overrides don't cascade into overlay-mounted popups.**~~ Closed — see [completed-backlog.md § 18.H](completed-backlog.md). New `Visual.AttachOverlayChild` / `DetachOverlayChild` API splits the popup's visual hop (renders into the host's `OverlayLayer`) from its logical hop (logical parent = the control that opened it). All 10 overlay-mounting controls migrated; resource walks / DataContext / inheritable-DP cascades now flow through the owner. OverlayLayer reports empty `logicalChildren` so logical-tree walks don't mistakenly route through it.

18.11. **No adaptive triggers in popup chrome templates.** [theme-architecture.md § Slice 4](theme-architecture.md#L533) calls for `when (Density = Compact)` / `when (PrefersContrast = More)` / `when (Pointer = Coarse)` triggers across templates that should adapt to the ambient inherited DPs. `DefaultMenuButtonPopup`, `DefaultContextMenuPopup`, and `DefaultSplitButtonPopup` ([framework.resources.mu](src/resources/framework.resources.mu)) carry none. M3 spec specifically calls for thicker borders on `PrefersContrast=More` — popup chrome currently fixed at `BorderThickness=(1)`. Density / pointer changes mostly affect the popup *items* (which are consumer-supplied), so the popup *chrome* is a lower priority than per-item templates. Adds to the contour described at § 18.6 (audit batches) and § 17.7 (adaptive trigger coverage gaps). Fix: one-line additions per template (e.g. `when (PrefersContrast = More) { PART_PopupBody.BorderThickness = (2); }`) under the Slice 4 wave.
