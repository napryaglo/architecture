...
5.6. **Missing WPF-roster panels.** `StackPanel`, `Canvas`, `DockPanel`, `VirtualizingStackPanel` are in. Three common WPF panels are not yet implemented — adding them rounds out the layout vocabulary so `ItemsControl.ItemsPanel` (and direct child hosting) can express the standard repertoire from markup:
  ...
  - **`Grid`** — column / row definitions with three sizing modes (`Auto`, `Pixel`, `Star`) plus shared-size groups. Star sizing distributes leftover space proportionally after `Auto` / `Pixel` measure. Attached properties `Grid.Row`, `Grid.Column`, `Grid.RowSpan`, `Grid.ColumnSpan` already conceptually fit the attached-property machinery from 5.1. The measure pass is the involved one — three sub-passes (auto sizing, pixel allocation, star distribution) interact with shared-size groups.

  - **`UniformGrid`** — N rows × M columns of equal-sized cells. Far simpler than `Grid`: child count + `Rows` / `Columns` DPs determine cell layout (defaults derive missing dimension from child count + the other). Used in calculator keypads, color pickers, simple game boards.

  All three are pure layout panels — no chrome, no triggers. They plug into the existing `ItemsControl.ItemsPanel` slot the same way `StackPanel` does today, and are now reachable from markup via the `itemspaneltemplate` resource form / inline syntax added in this branch.
...
8.5. **Adorner-style drop position indicators inside `ItemsControl` (drag-to-reorder).** Generalize the v1 D&D into a list-reorder helper: when a draggable is dragged over an `ItemsControl` with the reorder behavior installed, the framework computes the insertion position (between which two existing containers the cursor sits) and renders a thin "insertion line" between them via an adorner-like overlay. On drop, the `ItemsControl` moves the source's item to that index in the underlying `Items` / `ItemsSource`. The hard part isn't the visual (it's a thin `<line>` in the overlay layer); it's the contract — does the helper require the source's `DataObject` to carry the item identity? Does it move items in `Items` directly, or call a VM-supplied `OnReorder(fromIndex, toIndex)` callback? Pairs with WPF's `ItemsControl.ItemContainerStyle` + drag-handle conventions. Concretely, a "MoveItemsBehavior" or "ListBoxReorder" attached behavior + an `ItemsControl.AllowReorder` DP.

## 9. Architectural notes (for orientation)
