...
5.4. **No layout / render pipeline.** `Model.MarkMeasureDirty` / `MarkArrangeDirty` / `MarkRenderDirty` are no-op stubs. The metadata-flag routing in `OnPropertyChanged` fires them on every effective-value change, but nothing consumes those calls. A real layout pass (measure → arrange) and render queue need to be built. Tree primitives are in place: `Model` carries a parent link via `Attach`/`Detach`; `Single` (one-child slot) and `PanelBase` (children collection) extend `Model` with the structural API. Originally drafted as a separate `Visual` class hierarchy, then merged into `Model` so the parent link, dirty hooks, and metadata-flag routing apply uniformly.

5.5. **No concrete control / panel primitives.** No `Border`, `Grid`, `StackPanel`, `Button`, `TextBlock`, etc. The framework currently has only the abstract building blocks (`Model`, `Single`, `PanelBase`) and the property/binding system. Probably the next major chunk of work once the layout/render pipeline (5.4) exists.
...
- **`Visual` class was drafted then merged into `Model`.** Originally split as a layer above Model (tree primitives + layout-flag routing); collapsed back in because the parent link, `MarkX` hooks, and metadata-flag routing in `OnPropertyChanged` are uniform across all Models. Bare `Model` is concrete; `Single` and `PanelBase` add child slots.
- **`OnPropertyChanged` virtual signature.** Now `(descriptor, old, new)` — receives the `PropertyDescriptor` directly. Internal callback path (`EVD.SetInternalCallback`) carries the descriptor so cross-class properties (5.1) dispatch without re-lookup. User-facing `PropertyChangedCallback` signature (passed to `Add/RemovePropertyChangedListener`) is unchanged — still `(model, name, old, new)`.
...
- **Module split.** `metadata.ts` (flags + predicates), `property-descriptor.ts` (schema), `effective-value.ts` (per-instance value + listeners + callbacks), `binding.ts` (`PropertyPath` + `Binding` + `ValueConverter` + `BindingOptions`), `model.ts` (`Model` + `Single` + `PanelBase` + `PropertyKey`). Barrel: `index.ts`.
