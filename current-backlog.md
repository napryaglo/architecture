...
- **General `.Member: { … }` dotted aggregate-property block** — a `MemberBlock` AST node ([ast.ts](src/compiler/ast.ts)) with two strategies chosen by body shape ([compiler `compileMemberBlock`](src/compiler/compiler.ts)): **list** — bare elements append to `target.<Member>` via `.Add(child)` (ObservableCollection DP), reproducing the bespoke `ColumnDefinitions {}` lowering; **dictionary** — keyed entries (`@Key = value`, or an element with `x:key`) set into `target.<Member>` via `.Set(key, value)` (the ResourceDictionary-shaped contract, the same keyed surface `resources:` uses). A body becomes a dictionary as soon as one entry is keyed, and then every entry must be keyed. Named members (`services`) still branch for bespoke lowering.
...
- **`.Member:` dictionary strategy** ✅ — keyed entries lower to `Member.Set(key, value)` (see the `.Member:` layer above). The bigger *fold `resources:` into `.Member:`* ambition is split out to § 25 — it's a resources-block refactor, not a services concern.
...
**Future / not scheduled.** § 24 shipped a generic dictionary `.Member:` strategy (`@Key = value` / `x:key` → `target.<Member>.Set(key, value)`), which is the same keyed surface `resources:` populates a `ResourceDictionary` with. In principle `resources: { … }` could become `.Resources: { … }` over that one general mechanism, retiring the bespoke `compileResourcesBlock`/`compileResourcesSlot` paths.

Out of scope for now — `resources:` carries machinery the generic form doesn't yet cover (typed `x:key` accessors on the emitted dictionary class, `include` / `glyphs` splices, Style/Template implicit-key handling, theme bundling). A migration must preserve all of it. Tackle as its own refactor with a full resources test sweep; until then the generic dictionary `.Member:` and the bespoke `resources:` coexist.
