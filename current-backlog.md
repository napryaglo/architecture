...
Surfaced by the post-shipping self-review against [m3-modernization-plan.md](m3-modernization-plan.md) (Phases 0-9 + Appendix C + Phase 2.5 / Phase 3.5 Tier-1 close-out) and the architecture review of the SplitButton popup-template fix against [theme-architecture.md](theme-architecture.md). 18.6 is plan-process debt. 18.9 lists the plan-explicit deferrals. 18.12 is the residual theme-architecture-level caveat from the SplitButton popup conformance work. Fully-closed entries 18.1 / 18.2 / 18.3 / 18.4 / 18.5 / 18.7 / 18.8 / 18.10 / 18.11 / 18.13 moved to [completed-backlog.md § 18](completed-backlog.md) (18.A-18.P).

**Status roll-up** (last updated 2026-07-09):
- ✅ **Done → moved to [completed-backlog § 18](completed-backlog.md):** 18.2 (Button.CornerRadius → 18.L), 18.3 (Panel.ArrangeChild transition → 18.P), 18.4 (Storyboard.AwaitCompleted → 18.M), 18.7 (token audit → 18.N), 18.8 (Badge/Banner/BottomSheet demos → 18.O), 18.13 (typography → 18.K).
- 🟡 **Partial (still open below):** 18.6 (audit done + 7 Tier-1/2 controls given density/coarse triggers; toggles + ribbon/shell/container families remain), 18.9 (BottomAppBar shipped — 1 of 6; LoadingIndicator/SideSheet/DatePicker/TimePicker/Carousel remain), 18.12 (SplitButton.PopupTemplate + MenuButton.TriggerTemplate done; MenuButton.Template + Drawer residuals).
...
18.6. **M3 audit batches A-E never tracked as discrete commits.** [m3-modernization-plan.md § Appendix B](m3-modernization-plan.md) calls for 19 per-control audit commits before each phase — Batch A (4) before Phase 2, Batch B (4) before Phase 7, Batch C (2) before Phase 8, Batch D (5) before Phase 9, Batch E (4) anywhere across Phases 5-9. Each audit applies an 8-point checklist (tokens-only, state-layer pattern, five-state pass, shape tokens, typography roles, density triggers, coarse-pointer targets, live theme swap). The work landed alongside its consuming phase commit but no commit message records which controls actually passed all 8 checks. Fix: one-pass audit per-control with a per-control commit if any check fails. Pairs with 17.7 (adaptive trigger coverage gaps) which lists the controls still missing density / pointer / contrast triggers.
  ...
  **Progress (audit done + Tier-1/2 adaptive-trigger fixes landed):** full 8-point audit ran across all 22 `*.template.mu` files (62 controls). **Checks 1-5 + 8 pass at scale**; the systemic gaps are **check 6 (density)** and **check 7 (coarse-pointer)** — ~27 controls missing both. This pass added density (Compact/Comfortable) + coarse-pointer triggers to the **Tier-1/2 accessibility controls** across 5 files: NavigationItem + ActivityBarItem ([navigation](src/framework/navigation/navigation.template.mu)), TabItem ([tabs](src/framework/tabs/tabs.template.mu)), SegmentedItem + SplitButton ([button-groups](src/framework/button-groups/button-groups.template.mu)), Chip ([markers](src/framework/markers/markers.template.mu)), SearchBar ([search-bar](src/framework/search-bar/search-bar.template.mu)) — each targeting the content-padding/size-bearing part, scaled from the resting value on the 4dp grid. Container controls (NavigationRail/Bar, TabControl) correctly delegate to their per-item templates; Divider/Badge skipped (non-interactive).
  - **Toggles residual:** Switch / Checkbox / RadioButton were attempted but **reverted** — their painted parts (PART_Track/PART_Box/PART_Ring) hold margin-positioned children (thumb/checkmark/dot), so padding them insets/clips the mark instead of enlarging a hit area. They need a **structural** fix (an outer hit-target wrapper Border, or the control setting its own MinHeight), not a padding trigger. Tracked as a follow-up.
  - **Still pending (lower priority):** Ribbon family (9), shell chrome, notifications (ProgressIndicator/Banner/Snackbar), Card/Dialog/BottomSheet/Drawer containers, TopAppBar variants, Tooltip; plus coarse-only gaps on ColorPicker/BrushPicker/ThemeSelector.
...
18.9. **Plan-explicit M3 spec deferrals — pending demo motivation.** Listed for completeness; each one ships when a concrete demo motivates it. **Locked decision (user, this session): build ALL six** — sequenced smallest-first (BottomAppBar → LoadingIndicator → SideSheet → DatePicker → TimePicker → Carousel), each with its own demo. DatePicker/TimePicker/Carousel are large (calendar grid / clock face / scroll physics) and land across multiple commits/sessions.
  ...
  - ⬜ **`LoadingIndicator`** (M3 2024, distinct from `ProgressIndicator.Circular`). A new explicit "still loading" indicator; headline difference from mural's existing Linear/Circular progress is its variable-amplitude oscillation pattern. *Next up (smallest remaining).*
  - ⬜ **`SideSheet`** (Modal + Standard, [Appendix A](m3-modernization-plan.md)). M3's docked side panel — closest mural analog is `Drawer`; pairs with the Drawer Template-DP migration flagged in 18.12.
  - ⬜ **`DatePicker` / `TimePicker`** ([§ 8.10](m3-modernization-plan.md)). Modal + Docked variants per M3 spec — large (calendar grid / clock face).
