...
const KEYWORD_DOCS: ReadonlyMap<string, string> = new Map([
    ['Style',                    'Implicit or keyed Style. Implicit when only `TargetType=…` is given; keyed when `x:key="…"` is present. Body is a setter list plus `when` triggers.'],
    ...
    ['HierarchicalDataTemplate', 'DataTemplate with an `itemsselector` meta-attr — supplies child items for TreeView-style hierarchical content.'],
    ['ItemsPanelTemplate',       'Panel factory for an ItemsControl. Body is a single Panel element; no required meta-attrs.'],
    ['def',                      'Macro definition. Holes are `#1`, `#name`, … referenced from the macro body.'],
    ['when',                     'Trigger clause inside a Style or Template body. Supports `not`, `and`, `or` with normal precedence.'],
    ['on',                       'Event-trigger clause (`on Click { … }`) — fires storyboard/setter actions on a routed event.'],
    ...
    ['as',                       'Alias clause for `include` — keys the spliced resource (`include "…" as @Key`).'],
    ['include',                  'Compile-time splice of another file into the surrounding resource dictionary (e.g. an SVG converted to a Geometry).'],
    ['glyphs',                   'Bakes font-glyph outlines into a resource dictionary as PathGeometry at compile time, by glyph name or `= "\\uXXXX"` codepoint.'],
    ['fonts',                    'Declares font families from file sources (`Family from "…" [Weight=…, Style=…]`) referenced by later `glyphs` blocks and FontFamily uses.'],
